...
struct xenstore_client {
	...
	/*
	 * notify_cursor: next watcher to visit while a dispatch is in flight.
	 * xs_watcher_unregister() advances it when the node it points to is
	 * removed, so the dispatch loop never follows a dangling link.
	 */
	sys_snode_t *notify_cursor;
	/*
	 * notify_active: watcher selected for callback while the list lock is dropped.
	 * Unregister waits on that watcher's semaphore when another thread owns
	 * this callback.
	 */
	sys_snode_t *notify_active;
	bool notify_active_waiting;
	k_tid_t notify_dispatch_tid;
...
{
	return xs && !atomic_get(&xs->failed) && (xs->domint != NULL);
}
...
static inline int ring_avail_for_read(struct xenstore_client *xs, size_t *avail)
{
...
static inline int ring_avail_for_write(struct xenstore_client *xs, size_t *avail)
{
	struct xenstore_domain_interface *intf = xs->domint;
...
static int ring_write_all(struct xenstore_client *xs, const void *buf, size_t len,
			  k_timepoint_t deadline, bool *frame_started)
{
	const uint8_t *p = buf;
	size_t written = 0;
	...
	while (written < len) {
		...
		/* Large frames may need the peer to consume this chunk before we can continue. */
		notify_evtchn(xs->local_evtchn);
		written += rc;
	}
...
static void fail_pending_responses(struct xenstore_client *xs, int err);

static void xs_client_teardown(struct xenstore_client *xs)
{
	k_spinlock_key_t key;
	int mutex_err;
	...
	/*
	 * Synchronize with an event callback that may already have loaded our
	 * callback before the channel was unbound.
	 */
	key = k_spin_lock(&xs->lock);
	k_spin_unlock(&xs->lock, key);

	if (k_current_get() == k_work_queue_thread_get(&xs->workq)) {
		(void)k_work_cancel(&xs->event_work);
	} else {
		(void)k_work_cancel_sync(&xs->event_work, &xs->event_work_sync);
	}
...
static void finish_client_failure(struct xenstore_client *xs, int err,
				  bool teardown)
{
	if (teardown) {
		xs_client_teardown(xs);
	}

	fail_pending_responses(xs, err);
}

static void fail_client(struct xenstore_client *xs, int err)
{
	finish_client_failure(xs, err, atomic_cas(&xs->failed, 0, 1));
}
...
static int prepare_response(struct xenstore_client *xs, size_t *avail)
{
	...
	/*
	 * Stage the whole payload regardless of the waiter's buffer size so
	 * that XS_ERROR strings can always be parsed; whether the payload fits
	 * the waiter's buffer is decided at copy time in response_to_request().
	 */
...
static void dispatch_watcher_callback(struct xenstore_client *xs)
{
	...
	while (node) {
		...
		/*
		 * Publish the resume point: if the next watcher is removed
		 * while the lock is dropped, xs_watcher_unregister() advances
		 * the cursor for us.
		 */
		xs->notify_cursor = sys_slist_peek_next(node);
		xs->notify_active = node;
		xs->notify_dispatch_tid = k_current_get();
		cb = w->cb;
		param = w->param;
		...
		waiting = xs->notify_active_waiting;
		xs->notify_active = NULL;
		xs->notify_active_waiting = false;
		xs->notify_dispatch_tid = NULL;
		if (waiting) {
			k_sem_give(&w->idle);
		}

		node = xs->notify_cursor;
	}

	xs->notify_cursor = NULL;
	k_spin_unlock(&xs->lock, key);
...
static void response_to_request(struct xenstore_client *xs)
{
	...
	if (pending) {
		...
		if (err == 0) {
			if (xs->work_resp.pos > pending->len) {
				pending->err = -ENOSPC;
			} else {
...
static void fail_pending_responses(struct xenstore_client *xs, int err)
{
	...
	SYS_SLIST_FOR_EACH_NODE(&xs->resp_list, node) {
		struct xenstore_response *resp = CONTAINER_OF(node, struct xenstore_response, node);
...
static void event_work_handler(struct k_work *work)
{
	...
	while (true) {
		ret = ring_avail_for_read(xs, &avail);
		if (ret < 0) {
			fail_client(xs, ret);
			break;
		}
		...
		if ((ret == -EAGAIN) || (ret == -ENOMSG)) {
			...
			if (xs->to_discard && xs_is_initialized(xs)) {
				continue;
			}
			   ...
			   uint32_t req_id, uint32_t tx_id)
{
	...
	for (size_t i = 0; i < param_num; i++) {
		size_t add = calc_param_write_size(params, param_lens, param_num, i);
			 ...
			 k_timepoint_t deadline, bool *frame_started)
{
	...
	err = ring_write_all(xs, hdr, sizeof(*hdr), deadline, frame_started);
	if (err < 0) {
	...
	for (size_t i = 0; i < param_num; i++) {
		...
		err = ring_write_all(xs, params[i], param_len, deadline, frame_started);
		if (err < 0) {
			  ...
			  uint32_t req_id, uint32_t tx_id, k_timepoint_t deadline)
{
	...
	mutex_err = k_mutex_lock(&xs->req_mutex, sys_timepoint_timeout(deadline));
	if (mutex_err != 0) {
	...
	err = ring_avail_for_write(xs, &avail);
	if (err < 0) {
		teardown = atomic_cas(&xs->failed, 0, 1);
		goto end;
	}
	...
	if (avail == 0) {
		while (avail == 0) {
			if (sys_timepoint_expired(deadline)) {
				LOG_ERR("ring_write: timed out waiting for space");
				err = -ETIMEDOUT;
				goto end;
			}
	...
	err = write_request(xs, &hdr, params, param_lens, param_num, deadline, &frame_started);
	if ((err < 0) && ((err != -ETIMEDOUT) || frame_started)) {
		/*
		 * A partially written frame desynchronizes the request stream,
		 * and ring writes only fail when the shared indexes are
		 * corrupt; the connection is unrecoverable either way.
		 */
		teardown = atomic_cas(&xs->failed, 0, 1);
	}
	...
	if (teardown) {
		finish_client_failure(xs, err, true);
	}
...
int xs_init(void)
{
	...
	if (xs_cli.workq_started) {
		(void)k_work_cancel_sync(&xs_cli.event_work,
					 &xs_cli.event_work_sync);
	}
	...
	xs_cli.notify_cursor = NULL;
	xs_cli.notify_active = NULL;
	xs_cli.notify_active_waiting = false;
	xs_cli.notify_dispatch_tid = NULL;
	...
	while (true) {
		...
		err = ring_avail_for_read(&xs_cli, &avail);
		if (err < 0) {
			xs_client_teardown(&xs_cli);
			return err;
		}
		...
		(void)ring_read(&xs_cli, NULL, avail);
	}
	...
	/* Catch frames that arrived between the ring drain and the evtchn bind. */
	k_work_submit_to_queue(&xs_cli.workq, &xs_cli.event_work);
...
int xs_watcher_register(struct xs_watcher *w)
{
	...
	SYS_SLIST_FOR_EACH_NODE(&xs_cli.notify_list, node) {
		if (node == &w->node) {
...
int xs_watcher_unregister(struct xs_watcher *w)
{
	...
	/* Keep an in-flight dispatch from resuming at the node we remove. */
	if (xs_cli.notify_cursor == &w->node) {
		xs_cli.notify_cursor = sys_slist_peek_next(&w->node);
	}

	removed = sys_slist_find_and_remove(&xs_cli.notify_list, &w->node);
	if (removed && (xs_cli.notify_active == &w->node) &&
	    (xs_cli.notify_dispatch_tid != k_current_get())) {
		xs_cli.notify_active_waiting = true;
		wait_active = true;
	}
	k_spin_unlock(&xs_cli.lock, key);
...
static ssize_t xs_cmd_no_param_timeout(enum xsd_sockmsg_type type, const char *path, char *buf,
...
static ssize_t xs_cmd_str_param_timeout(enum xsd_sockmsg_type type, const char *path,
					const char *str, char *buf, size_t len, uint32_t tx_id,
					k_timeout_t tout)
{
