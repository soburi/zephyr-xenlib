...
static int read_payload(struct xenstore_client *xs, size_t avail)
{
	...
	if ((xs->work_resp.len > 0) && (xs->work_resp.pos < xs->work_resp.len)) {
		/* Ensure callers see a NUL-terminated payload when space allows. */
		xs->work_resp.buf[xs->work_resp.pos] = '\0';
	}

	return 0;
}

static void xs_deliver_watch_notifications(struct xenstore_client *xs, const char *path,
					   const char *token)
{
	k_spinlock_key_t key;
	sys_snode_t *node;
 ...
 */
static void finalize_response(struct xenstore_client *xs)
{
	...
	xs_deliver_watch_notifications(xs, path, token);
	xs_frame_reset(xs);
}
...
static void response_to_request(struct xenstore_client *xs)
{
	...
	if (pending) {
		...
		if ((pending->len > 0) && (pending->pos < pending->len)) {
...
static int xs_prepare_request(struct xsd_sockmsg *hdr, enum xsd_sockmsg_type type,
...
static int xs_write_request_locked(struct xenstore_client *xs, const struct xsd_sockmsg *hdr,
			  ...
			  uint32_t req_id, uint32_t tx_id)
{
	...
	err = xs_prepare_request(&hdr, type, params, param_num, req_id, tx_id);
	if (err < 0) {
		return err;
	}
	...
	err = xs_write_request_locked(xs, &hdr, params, param_num);
			       ...
			       uint32_t tx_id, k_timeout_t timeout)
{
	...
	err = submit_request(xs, type, params, params_num, buf, len, resp_local.req_id, tx_id);
	if (err < 0) {
