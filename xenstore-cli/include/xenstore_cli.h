 ...
 * A watcher can be registered at most once at a time and must remain valid
 * from registration until ::xs_watcher_unregister() returns.
 ...
 * Safe to call multiple times, but not safe to call concurrently from multiple
 * threads.
 *
 ...
 *
 * After this function returns, the callback will not be invoked for
 * subsequent watch events and @p w may be reused or freed. It is safe to call
 * this from the watcher's own callback. If called from another thread while a
 * callback for @p w is in flight, this function waits until that callback is
 * no longer using the watcher descriptor.
 *
 * Note that this only removes the local callback; use xs_unwatch() to remove
 * the watch registration from the XenStore server.
 *
 * @param w Watcher descriptor previously registered with xs_watcher_register().
 *
 * @retval 0       Succeeded in unregistering the watcher.
 * @retval -EINVAL @p w is NULL.
 * @retval -ENOENT @p w was not registered.
 */
int xs_watcher_unregister(struct xs_watcher *w);

/**
 * Response buffer contract for the APIs below:
 *
